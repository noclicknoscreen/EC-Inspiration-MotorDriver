...
void UDPRead() {
  ...
  char strBuffer[255]; //buffer to hold incoming packet
  ...
  if (packetSize) {
    ...
    int len = Udp.read(strBuffer, 255);
    if (len > 0) {
      strBuffer[len] = 0;
    }
    ...
    /*
    if (strBuffer.startsWith("position=")) {
      // decodeAnimation(m);
      analogWrite(LED_BUILTIN, shutterPosition * 255);
    } else {
      // UNKNOWN --------------------
    }
    */
